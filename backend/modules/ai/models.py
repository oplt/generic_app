...
# Compatibility registration only. Active document services use RAG-owned
# models; these legacy tables are read-only until the migration milestone is
# complete. New code must not import them as an ownership boundary.
from backend.modules.ai.legacy_models import AiDocument, AiDocumentChunk  # noqa: F401
...
class AiRun(Base):
    ...
    provider_last_status_code: Mapped[int | None] = mapped_column(Integer, nullable=True)
    provider_last_error: Mapped[str | None] = mapped_column(Text, nullable=True)
    provider_deadline_at: Mapped[datetime | None] = mapped_column(
...
class AiEvaluationCase(Base):
    ...
    evaluation_type: Mapped[str] = mapped_column(String(32), default="standard")
