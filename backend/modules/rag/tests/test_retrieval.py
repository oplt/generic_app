...
class RetrievalFilterTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retrieval_filters_by_user_id(self):
        self.adapter.repo.similarity_search_indexed = AsyncMock(
            return_value=[
                RetrievedChunk(
                    chunk_id="c1", document_id="doc-a", content="user a secret", score=0.95,
    ...
    async def test_unavailable_pgvector_fails_closed(self):
        with self.assertRaisesRegex(RuntimeError, "pgvector retrieval is unavailable"):
            await self.adapter.similarity_search(
                "query", user_id="user-a", project_id=None, top_k=5, query_embedding=[1.0, 0.0]
            )
