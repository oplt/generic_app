...
import { Box, Skeleton, Stack, Tab, Tabs, Tooltip, Typography } from "@mui/material";
import {
...
function ShowingCaption({ shown, total, noun }: { shown: number; total: number; noun: string }) {
    const text = `${shown} of ${total} ${noun}`;
    return (
        <Tooltip title={`Showing ${text}`}>
...
export default function AiStudioPage() {
    ...
    if (m.isError || !m.overview) {
        return (
            <PageShell maxWidth="xl">
                <SettingsTabs />
                <QueryErrorAlert
                    error={m.error ?? new Error("Failed to load AI studio overview.")}
    ...
    const pendingReviews = m.reviews.filter((item) => item.status === "pending").length;
        ...
        <PageShell maxWidth="xl">
            ...
            >
                <StatCard
                    ...
                    value={m.overview.prompt_templates_count}
                    description={
                        <ShowingCaption
                            shown={m.promptTemplates.length}
                            total={m.overview.prompt_templates_count}
                            noun="templates"
                        />
                    }
                    icon={<PromptIcon />}
                ...
                <StatCard
                    label="Documents"
                    value={m.overview.documents_count}
                    description={
                        <ShowingCaption
                            shown={m.documents.length}
                            total={m.overview.documents_count}
                            noun="sources"
                        />
                    }
                    icon={<DocumentIcon />}
                ...
                <StatCard
                    label="Datasets"
                    value={m.overview.datasets_count}
                    description={
                        <ShowingCaption
                            shown={m.datasets.length}
                            total={m.overview.datasets_count}
                            noun="datasets"
                        />
                    }
                    icon={<DatasetIcon />}
