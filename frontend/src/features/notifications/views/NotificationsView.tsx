...
export default function NotificationsPage() {
    ...
    const markOneMutation = useMutation({
        ...
        onMutate: async (id) => {
            ...
            const wasUnread = previous?.some((item) => item.id === id && !item.is_read);
            if (wasUnread) {
                queryClient.setQueryData(queryKeys.notifications.unreadCount, {
                    count: Math.max(0, (previousUnread?.count ?? 0) - 1),
                });
            }
    ...
    const unreadCount = unreadCountQuery.data?.count ?? notifications?.filter((item) => !item.is_read).length ?? 0;
